<?php
/**
* Telegram Bot example for dovedormireinumbriabot
 ...
 function shell($telegram,$text,$chat_id,$user_id,$location,$reply_to_msg)
{
	date_default_timezone_set('Europe/Rome');
	$today = date("Y-m-d H:i:s");
	if (strpos($text,'@dovedormireinumbriabot') !== false) $text=str_replace("@dovedormireinumbriabot ","",$text);

	if ($text == "/start" || $text == "©️ Informazioni") {
		$img = curl_file_create('logoum.png','image/png');
		$contentp = array('chat_id' => $chat_id, 'photo' => $img);
		$telegram->sendPhoto($contentp);
		$reply = "Benvenuto. Per ricercare una struttura ricettiva della Regione Umbria, censita dal Dipartimento regionale del turismo dello sport e dello spettacolo, digita il nome del Comune oppure clicca sulla graffetta (📎) e poi 'posizione' . Puoi anche ricercare per parola chiave nel titolo anteponendo il carattere ?. Verrà interrogato il DataBase openData utilizzabile con licenza CC-BY presente su http://dati.umbria.it/dataset/strutture-ricettive. In qualsiasi momento scrivendo /start ti ripeterò questo messaggio di benvenuto.\nQuesto bot è stato realizzato da @piersoft con la preziosa collaborazione di @mauroferri e potete migliorare il codice sorgente con licenza MIT che trovate su https://github.com/piersoft/dovedormireinemiliaromagnabot.\nLa propria posizione viene ricercata grazie al geocoder di openStreetMap con Lic. odbl.\nImmagine derivata da wikipedia lic. CC-BY-SA\n";
		$content = array('chat_id' => $chat_id, 'text' => $reply,'disable_web_page_preview'=>true);
		$telegram->sendMessage($content);
		$log=$today. ",new chat started," .$chat_id. "\n";
		file_put_contents('/usr/www/piersoft/dovedormireinumbriabot/db/telegram.log', $log, FILE_APPEND | LOCK_EX);
...
if(strpos($text,'!') !== false) {
	...
	//			sleep (1);
		...
		//		sleep (1);
			...
			$homepage="";
$json_string=file_get_contents("/usr/www/piersoft/dovedormireinumbriabot/db/ricettive.json");
...
//	if (strpos($filterint,$textint) !== false ){
	...
	if(strpos($parsed_json->{'features'}[$i]->{'geometry'}->{'coordinates'}[1],'.') !== false){
